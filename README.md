...
**Go 1.26 + Gin + HTMX + SQLite（組込ファイル型 DB）** で構成された農業IoTシステムです。
ESP32 + SHT31（温湿度センサー）のデータを Go バックエンドで受信・蓄積し、ブラウザで可視化・アラート管理します。
DB サーバもコンテナ実行基盤も不要で、**単一の Windows `.exe` をダブルクリックするだけ**で、畑のノートPC 上で完結するデスクトップアプリとして動かせます（旧 PostgreSQL 構成から SQLite へ移行済み）。

> **アーキテクチャの要点**
> フロント用 JSON API を持たず、**templ が HTML を直接返す**サーバサイドレンダリング（SSR）構成です。
> 部分更新は **HTMX**、軽い UI 状態は **Alpine.js** で扱い、JS ビルドは行いません。
> データは一方向パイプライン **デバイス →（HTTP + Bearer）→ API → SQLite → templ → ブラウザ** で流れます。
> DB は `modernc.org/sqlite`（pure-Go・cgo 不要）で、アプリと同梱の単一実行ファイルに収まります。
...
> **DB サーバ・コンテナ実行基盤は不要です。** DB は SQLite（組込ファイル型）で、アプリ起動時に
> DB ファイルを自動作成しスキーマを自動適用します。別途のデータベースサーバを立てる必要はありません。
>
> **PHP / Composer / Node.js も不要です。**
...
> **macOS をお使いの方へ**
> macOS には `make` がプリインストールされています。**Go と Git** を入れれば、`make setup` ですぐ始められます。
...
> | `localhost:8080` にアクセスできない | Windows Firewall がポートをブロック。ポート 8080 の受信を許可する |
...
DB は SQLite（組込ファイル型）なので **DB サーバの起動は不要**です。`make dev` を実行すると、
起動時に DB ファイルが無ければ自動作成し、未適用のマイグレーションを自動適用します。
...
Go の依存モジュールをダウンロードします（`go mod download`）。env は任意のため `.env` の作成は必須ではありません。
...
**既存のアプリデータは削除されます**（`goose_db_version` は温存）。初回はマイグレーション適用後に実行してください
（`make dev` を一度起動するか、手動なら `make migrate-up`）。
...
起動後、ブラウザで **http://localhost:8080** にアクセスできます（ポートは `.env` の `APP_PORT`。未設定なら 8080）。
DB ファイルの場所は `DATABASE_URL`（未設定時はカレントの `go_iot.sqlite`）で決まります。

> **マイグレーションについて**: 本番（`.exe`）起動時は `internal/migrate` が go:embed 同梱のマイグレーションを
> 自動適用します。開発中に手動で適用・確認したい場合は `make migrate-up` / `make migrate-status` を使えます
> （`goose` が `db/migrations/*.sql` を順に適用し、6テーブル: users / devices / device_tokens / sensor_readings / alert_rules / alert_histories を作成）。
...
cd go_iot
make setup        # 依存DL
make dev          # SQLite 自動作成 + マイグレーション自動適用 + air 起動 → http://localhost:8080
make seed         # 開発データ投入（任意・別ターミナルで）
```
...
## デスクトップアプリとして配布する（単一 Windows `.exe`）

圃場のノートPC で「サーバ知識なし・ネット接続なし」で使えるよう、**単一の `.exe` をダブルクリックするだけ**で起動するデスクトップアプリとして配布できます。macOS / Linux 上から `CGO_ENABLED=0` でクロスビルドします（pure-Go・追加ランタイム不要）。

```bash
make build-windows       # コンソール窓あり（ログは標準出力）→ dist/go_iot.exe
make build-windows-gui   # コンソール窓なし（ログは %LOCALAPPDATA%\go_iot\app.log）→ dist/go_iot.exe
```

生成された `dist/go_iot.exe` を対象の Windows 端末へコピーして実行（ダブルクリック）すると、以下が自動で完了します。

1. **データ保存先の自動決定**: `%LOCALAPPDATA%\go_iot\`（書込制限のある `Program Files` 配下は使わない）。
2. **DB ファイルの自動作成とスキーマ自動適用**: `app.db` を作成し、起動時にマイグレーションを冪等適用。
3. **CSRF 認証鍵の自動生成・永続化**: 再起動後もログイン状態を維持。
4. **ポート自動採番**: 既定 8080 が使用中なら空きポートを自動選択。
5. **ブラウザ自動表示**: 既定ブラウザで `http://localhost:<実ポート>` を開く。
6. **mDNS でホスト名公開**: `go-iot.local` を同一 LAN へ公開し、DHCP で IP が変わっても ESP32 から到達可能。

> **ESP32 からの送信**: 同一 LAN 上の ESP32 は `go-iot.local`（または LAN IP）の `POST /api/sensor-data` へ Bearer トークン付きで送信します。インターネット接続は不要です。
>
> **env は任意**: 環境変数・`.env` が無くても起動します。保存先や鍵・ポートを固定したい場合のみ `DATABASE_URL` / `SESSION_SECRET` / `APP_PORT` を指定してください（`.env.example` 参照）。
>
> **残存リスク（別フェーズ検証）**: 実 Windows 端末＋実 ESP32 での通し検証、GUI ビルドのコンソール窓非表示の実機確認は本リポジトリの自動テスト範囲外です（クロスビルド成功・PE32+ の console/GUI 生成・ロジック/結合テストまでを CI で担保）。
...
├── internal/
│   ├── config/               環境変数の読込・検証（env 任意・既定値を appdata から解決）
│   ├── appdata/              アプリデータ保存先の単一解決源（%LOCALAPPDATA%\go_iot 等）
│   ├── domain/               Metric / ComparisonOperator Enum（純粋・無依存層）
│   ├── infra/{db,pgconv,token}/  SQLite 接続(modernc) / 数値量子化 / トークン生成
│   ├── migrate/              go:embed 同梱マイグレーションの起動時自動適用（goose ライブラリ）
│   ├── desktop/              ポート自動採番（listener 先取り）+ 既定ブラウザ自動起動
│   ├── mdns/                 安定ホスト名 go-iot.local の mDNS 公開（IP 変動再登録）
│   ├── applog/               ログ出力先解決（GUI=app.log / console=標準出力）
│   ├── auth/                  認証(authN)：device_auth.go（Bearer）/ session_auth.go（scs + SQLite）
│   ├── authz/                 認可(authZ)：所有者認可(BOLA 防止)を集約
│   ├── handler/               HTTP ハンドラ（Gin）
│   ├── repository/            sqlc 生成コード（Querier interface）
│   ├── dbsnapshot/            DB 内省＋スナップショット整形（introspect=DB依存 / render=純粋・テスト済）
│   ├── docs/                  OpenAPI YAML + Scalar UI（go:embed）
│   ├── service/               アラート判定等のサービス層
│   ├── middleware/            SessionLoad / MethodOverride / CSRF 等
│   └── view/                  templ（layout / component / page）+ 静的アセット配信
├── db/
│   ├── migrations/            goose マイグレーション（*.sql・SQLite 方言。単一正本）
...
├── other/                     補助ドキュメント
├── Makefile                   主要コマンド（build-windows でクロスビルド）
...
```bash
# --- 開発サーバ ---
make dev           # air でホットリロード開発サーバを起動（SQLite 自動作成・マイグレーション自動適用）

# --- ビルド / 実行 ---
make build              # templ generate → 本番用バイナリをビルド（./tmp/main）
make run                # ビルドして実行
make build-windows      # Windows 単一 .exe をクロスビルド（コンソールあり）→ dist/go_iot.exe
make build-windows-gui  # Windows 単一 .exe をクロスビルド（コンソール窓なし）→ dist/go_iot.exe
...
make db-snapshot   # SQLite を内省し docs/database_snapshot/ にテーブル定義 + ER図を生成（要 migrate-up 済み）
...
DB は **SQLite（組込ファイル型）の単一ファイル**です。別途のデータベースサーバ・コンテナは不要で、
接続層（`internal/infra/db`）が WAL / busy_timeout / foreign_keys を自動付与します。

| 項目 | 値（デフォルト） |
|------|------------------|
| ドライバ | `modernc.org/sqlite`（pure-Go・cgo 不要） |
| DB ファイル | 開発: `DATABASE_URL`（未設定時 `./go_iot.sqlite`）／配布 `.exe`: `%LOCALAPPDATA%\go_iot\app.db` |
| DSN 形式 | `file:` URI（例 `file:./go_iot.sqlite`） |
| ジャーナル | WAL（読取と書込を並行可能に） |

> 保存先を変えたい場合は `DATABASE_URL` に `file:` DSN を指定します（`.env.example` 参照）。
...
### DBeaver / sqlite3 から接続する場合

DB は単一ファイルなので、GUI/CLI から **DB ファイルを直接開く**だけです（サーバ接続不要）。

| 項目 | 値 |
|------|------|
| ドライバー | **SQLite** |
| ファイルパス | `DATABASE_URL` の `file:` 以降（開発時の既定: `./go_iot.sqlite`） |

```bash
# sqlite3 CLI で直接開く例（開発時の既定パス）
sqlite3 ./go_iot.sqlite
```
...
> **DB スキーマを知りたい AI / 開発者へ**: まず **`docs/database_snapshot/table_definitions.md`**（テーブル定義）と **`er_diagram.mmd`**（Mermaid ER 図）を読んでください。`make db-snapshot` で SQLite を内省して再生成される静的ファイルで、**DB に接続しなくてもカラム・型・デフォルト・索引・CHECK 制約・論理リレーションを把握できます**。マイグレーション（`db/migrations/`）を変更したら `make db-snapshot` で更新してください（生成ツールは `cmd/db-snapshot` + `internal/dbsnapshot`、整形ロジックはユニットテスト済み）。

> **再調査の前に読むべき1冊**: `2cc_sdd/実装現状サマリ.md`。
> 「今コードがどうなっているか」（あるべき姿ではなく現状スナップショット）を全サブシステム横断でまとめてあり、別セッション・別担当者がゼロから読み解く手間を削減します。**現状はバックエンド完成・フロントエンド（Web UI 層）未着手**です。
...
>
> **複数行コマンドは自動承認されない**: グロブ `*` は改行にマッチしません。複数引数のコマンドは**1行で**書いてください。
...
第一の配布形態は **単一 Windows `.exe`（SQLite 同梱・サーバ不要）** です（上の「デスクトップアプリとして配布する」を参照）。圃場のノートPC へコピーしてダブルクリックするだけで、DB 自動作成・スキーマ自動適用・ブラウザ自動表示まで完了します。

サーバ常駐させたい場合は、ホスト OS 向けにビルドしたバイナリをそのまま起動します（DB は SQLite ファイルなので追加のミドルウェアは不要）。

```bash
make build        # templ generate → ./tmp/main を生成
./tmp/main        # 環境変数（.env / 環境）を読んで起動（env 未設定でも起動可）
```

> `APP_ENV=production` のとき Gin は ReleaseMode で起動します。production では `SESSION_SECRET`（CSRF 認証鍵）は base64 表現で 32 文字以上のランダム値が必須です。
...
| 症状 | 対処 |
|------|------|
| `make migrate-up` がエラー | `DATABASE_URL`（`file:` DSN）が書込可能なパスか確認。未設定時は `./go_iot.sqlite` を使う |
| `database is locked` が出る | 別プロセスが同じ SQLite ファイルを開いている。多重起動を避ける（WAL + busy_timeout で通常は待機吸収） |
