...
func baseDeviceInfoView() DeviceInfoView {
	return DeviceInfoView{
		...
		Crop:         "ゴーヤ", // 栽培作物は日本語ラベル (handler が整形済・未設定は "未設定")
...
func TestDeviceInfoPanel_稼働中の全要素を描画(t *testing.T) {
	...
	// dl 項目ラベル
	for _, label := range []string{"名前", "MAC", "場所", "状態", "最終通信", "作物"} {
	...
	assertContains(t, html, "ゴーヤ")                  // 栽培作物の日本語ラベル (VPD 適正帯の根拠・センサー毎)
...
func TestDeviceInfoPanel_未通信と未設定のフォールバック表示(t *testing.T) {
	...
	v.Crop = "未設定"        // 栽培作物未選択 (既定しきい値で VPD を見る)
	html := render(t, DeviceInfoPanel(v))

	assertContains(t, html, "未通信")
	assertContains(t, html, "未設定")
	// 作物ラベルと未設定値の両方が描画されること (場所と同じフォールバック表記)。
	assertContains(t, html, "作物")
}
