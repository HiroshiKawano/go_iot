...
func TestEvaluateAndNotify_温度ルール発火で履歴1件とスナップショット(t *testing.T) {
	...
	if got := pgconv.NumericToFloat(p.Threshold); got != 35.00 {
		t.Errorf("Threshold: got %v, want 35.00", got)
	}
	// 実測値は温度の値（湿度と取り違えない）
	if got := pgconv.NumericToFloat(p.ActualValue); got != 36.0 {
		t.Errorf("ActualValue: got %v, want 36.0", got)
	}
	// triggered_at = 計測時刻
	if !pgconv.TimestamptzToTime(p.TriggeredAt).Equal(fixedRecordedAt) {
		t.Errorf("TriggeredAt: got %v, want %v", pgconv.TimestamptzToTime(p.TriggeredAt), fixedRecordedAt)
	}
}
...
func TestEvaluateAndNotify_湿度ルールは湿度を実測値に使う(t *testing.T) {
	...
	if got := pgconv.NumericToFloat(repo.created[0].ActualValue); got != 25.0 {
...
func TestEvaluateAndNotify_値域外指標のルールはスキップして継続(t *testing.T) {
	repo := &fakeAlertRepo{rules: []repository.AlertRule{
		{ID: 1, DeviceID: 1, Metric: "pressure", Operator: string(domain.OpGreaterThan), Threshold: pgconv.Numeric2(10), IsEnabled: true}, // 値域外
...
func TestEvaluateAndNotify_値域外演算子のルールはスキップ(t *testing.T) {
	repo := &fakeAlertRepo{rules: []repository.AlertRule{
		{ID: 1, DeviceID: 1, Metric: string(domain.MetricTemperature), Operator: "==", Threshold: pgconv.Numeric2(35), IsEnabled: true}, // 値域外演算子
	}}
	e := &AlertEvaluator{Repo: repo}
