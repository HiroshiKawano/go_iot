...
func TestValidation_露点パネルは研究用スコープに限定(t *testing.T) {
	...
	body := w.Body.String()
	// 農家向け平易表示・圃場共有 URL・病害アラート/通知・他指標(多地点等)を描画しない。
	// 注: GDD/積算温度 は gdd-forecast spec が、THI/絶対湿度 は heat-stress-thi spec が device-show に
	// 正式追加した兄弟パネルゆえ、本テストの除外対象から外す（スコープ境界の移動）。露点パネル自体が
	// 他指標へ越境しないことは依然として担保する（多地点/圃場間は本フェーズ対象外で除外語に残す）。
	for _, ng := range []string{
		"圃場共有", "病害警告", "シェア", "信号", // 農家向け平易表示・共有
		"アラートを発火", "通知を送信", // 病害アラート/通知の発火
		"不快指数", "多地点", "圃場間", // 他指標（THI/絶対湿度 は heat-stress-thi の正式パネルゆえ除外語から外す）
	} {
		if strings.Contains(body, ng) {
