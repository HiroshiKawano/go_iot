...
6. 部分 HTML レスポンスを返す Gin Handler を実装する
...
**Gin Handler 側 (`internal/handler/dashboard.go`):**
...
func (h *Readings) Index(c *gin.Context) {
    ...
    if err := page.Readings(device, readings, summary).Render(ctx, c.Writer); err != nil {
        c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"message": err.Error()})
    }
}
```
...
Gin Handler が返す部分 HTML レスポンスの一覧。
