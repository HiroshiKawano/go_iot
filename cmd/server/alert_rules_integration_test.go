...
// TestIntegration_アラートルールのミューテーションはCSRF欠如で419 は、追加/更新/有効切替/削除の
// 各ミューテーションが CSRF トークン欠如で 419 になることを検証する (要件 10.3 / gorilla/csrf)。
// CSRF 失敗は BOLA 認可拒否 (403) と区別するため 419 (middleware.StatusCSRFExpired) を返す。
// web グループの CSRF ミドルウェアは RequireAuth より先に評価されるため、未認証でも 419 になる。
// 同時に「ルートが解決し 404 でない」= 6 ルートが配線済みであることの証左でもある。
func TestIntegration_アラートルールのミューテーションはCSRF欠如で419(t *testing.T) {
	app := newTestHandler(t)
	cases := []struct {
		method, path string
	}{
	...
	for _, tc := range cases {
		...
		if w.Code != 419 { // middleware.StatusCSRFExpired
			t.Errorf("%s %s (CSRF 欠如) = %d, want 419", tc.method, tc.path, w.Code)
		}
	}
}
