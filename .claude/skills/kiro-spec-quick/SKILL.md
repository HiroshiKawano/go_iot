...
>
> **テストガイダンス集**: design（Phase 3）の Testing Strategy と tasks（Phase 4）のテストタスクは、`2cc_sdd/テストガイダンス集.md`（全50節・冒頭の `## Go テーマ別索引`→該当節）の Go テスト定石（Querier モック / `httptest`+gin / templ `Render` 検証 / gorilla/csrf 往復 / scs in-memory / カバレッジ80%設計 / 列挙防止 / 302・303）に沿って具体化する。実装（`/tdd`・`/kiro-impl`）の第一参照でもある。約370KB の丸読み禁止。requirements（Phase 2）では不要（WHAT/HOW 分離）。
