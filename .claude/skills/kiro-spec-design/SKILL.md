...
> 3. 列挙がなければ、索引から対象画面に該当する ★★★ 節を選んで読む（§2 モック→templ+HTMX 変換ルール / templ コンポーネント分割 / 命名規約、§3 id属性一覧、§4 画面別HTMX操作仕様、§7 バリデーションエラー表示、§8 CSRF、**§31 モッククラスそのまま使用・独自クラス新設禁止（CSS 流用の核）**、**§40-B CSS 単一ソース運用（`mocks/html/style.css` が正本・本番は生成物）**）。Tom Select を使う画面は §16・C12 も読む。
