...
| ★★★ | [Tom Select 障害パターン集（TS）](#16-tom-select-障害パターン集ts) | Tom Select と HTMX/Alpine.js/CSS の干渉で発生する**具体的な障害と回避策**（TS-1〜TS-13）。新パターン発見時に連番追記 |
| ★ | [複数インラインCRUD領域の共存](#15-複数インラインcrud領域の同一画面共存) | 1画面に複数のインラインCRUD領域がある場合の設計注意 |
| ★★ | [テーマ切替（ダークモード等）実装パターン](#66-テーマ切替ダークモード等実装時の-cssjs-設計パターンと落とし穴) | CSS ダーク2導線トークン再マップ・SVGアイコンのテーマ追従（`mask-image`罠）・サードパーティJSライブラリ（ECharts等）のクライアント側テーマpatch設計・ライブラリ既定値の実測手法 |
...
> - **障害症状（2026-06-28 sensor-data-export スモークで確認）: 「ウィジェットだけ消えて表は残る」＝ swap 中断のサイン。** `htmx:beforeSwap` の dispose は走ったが `htmx:afterSwap` が**発火しなかった**ときに起きる: サーバ停止・フラグメントが 4xx/5xx（§7 の `responseHandling` で `swap:false`）・ネットワークエラー（`htmx:sendError`）等で swap が中断すると、既存 DOM の表・カードは残るが ECharts/Tom Select だけが破棄されたまま再初期化されない。**「グラフが全部消えた」を見たら、まずフラグメント応答（200 か）とサーバ稼働を Network で疑う**（dispose 自体は正常動作なのでコンソールは無言になりがち＝コンソールエラーだけ見ると空振りする）。**リロードで復活すれば swap 中断が原因と確定**。コードのデグレと切り分けるには、初回ロード（`DOMContentLoaded` 経路）でウィジェットが描画されるかを別途確認する。
...
> - **markArea（固定しきい値帯）の JSONタグ非準拠罠（vpd-dashboard・2026-06-27）.** go-echarts/v2 の `opts.MarkAreaData.YAxis` は JSON タグが `json:"YAxis"`（**大文字始まり＝ECharts 非準拠**。ECharts は `yAxis` を期待）で、ペアビルダー `WithMarkAreaData0/1` も同型を使うため**ネイティブの y 軸範囲 markArea は塗りが一切出ない**。正しいタグの `opts.MarkAreaNameYAxisItem`（`json:"yAxis"`）は**単一点用**でペア（範囲帯）を構成できない。→ **markArea は go-echarts 型に頼らず、`json.Marshal(line.JSON())` の出力を `map[string]any` へ戻して `series[0]` に、正しい小文字 `yAxis` キーの markArea を自前注入**してから `encoding/json`（`SetEscapeHTML=true`）で再シリアライズし HTML 安全化する（実装 `internal/chart/vpd_echarts.go` の `injectVPDMarkArea`＝VPD 適正帯3ゾーン）。テストは option JSON に小文字 `"yAxis"` が含まれ大文字 `"YAxis"` が**不在**であることで固定（テストガイダンス集 §56.1）。同じ非準拠は markLine 等の範囲系にも及ぶ前提で扱う。なお3ゾーンの**意味の向き**は別問題（VPD は高=乾燥/低=多湿＝低VPD側が湿りすぎ・高VPD側が乾きすぎ。逆定義しても全テスト緑で通るため実機×ドメイン確認が要る＝テストガイダンス集 §5.6.AA）。
...
> - **凡例マーカー/シンボル色の lineStyle/itemStyle 二重指定罠（dewpoint-disease-risk・2026-06-29）.** line 系列に `WithLineStyleOpts(opts.LineStyle{Color:X})` **だけ**設定すると**線は X 色**で描かれるが、**凡例マーカーとデータ symbol は ECharts 既定パレットへフォールバック**する（露点パネルで気温重ね線=橙なのに凡例マーカーが緑になり実機スモークで発覚）。線色と凡例/symbol を揃えるには `WithItemStyleOpts(opts.ItemStyle{Color:X})` を**同値で併設**する（`lineStyle.color` と `itemStyle.color` の両方へ同じ色を渡す）。テストは option JSON で `series[n].itemStyle.color == lineStyle.color` をアサート（テストガイダンス集 §56.1）。自動テストは色トークンの存在までで「凡例マーカーの見た目不一致」は拾えず**実機スモーク（テストガイダンス集 §5.6.AA）で getOption/目視確認**が要る。
> - **時間区間（xAxis 範囲）markArea への横展開と「専用 injector」原則（dewpoint-disease-risk・2026-06-29）.** 上記 markArea 自前注入（小文字キーへの差し替え）は**時間区間の帯ハイライト**（結露帯・欠測ギャップ等）にも要る。`opts.MarkAreaData` は xAxis 側も非準拠ゆえ、ECharts 準拠の**小文字 `xAxis` キー**で markArea を自前構築し `series[0]` へ注入する（実装 `injectGapMarkArea`＝欠測ギャップ灰色帯で P5 data-quality-meta が確立、`injectCondensationMarkArea`＝結露帯 寒色 `rgba(66,99,235,0.12)` で P6 が踏襲）。**色・意味の違う帯を足すときは既存の共有 injector（`injectGapMarkArea` の灰色 `gapZone`）を改変せず専用 injector を新設する**（共有関数を太らせると他画面=P5 欠測帯が回帰する＝共有ヘルパは最小・不変に保ち画面固有条件をローカル関数で重ねる原則の go-echarts 版）。テストは小文字 `"xAxis"` 在・大文字 `"XAxis"` 不在で固定（テストガイダンス集 §56.1）。
> - **value 軸チャート（経過日数の数値軸＋`[x,y]` 座標ペア）の実装定石と2つの「テストが緑でも実描画が壊れる」罠（gdd-forecast・2026-06-29）.** GDD 累積曲線は温湿度/VPD/露点の**時刻 category 軸**と決定的に違い、x が**経過日数の value 軸**（予測到達日＝データ範囲外の未来を `markLine{xAxis:数値}` で表すため value 軸が必須）。実装 `internal/chart/gdd_echarts.go`。
>   - **`SetXAxis` を呼ばない.** value 軸では category データ列を持たない。`charts.WithXAxisOpts(opts.XAxis{Type:"value", Min:0, Max:…})` で軸を設定し、`line.SetXAxis(...)` は**省略**、series データは `opts.LineData{Value: []float64{x, y}}` の**座標ペア**で与える（go-echarts は `initXYAxis()` で `XAxisList[0]` を必ず確保し、`SetXAxis` 未呼出なら `Data` は nil＝`omitempty` で省略され value 軸として正しく出る）。
>   - **markLine/markPoint も自前注入（§1288 が予告した markLine への波及）.** 目標 GDD 水平線＝小文字 `yAxis`、予測到達日 垂直線＝小文字 `xAxis`、予測点＝`coord:[x,y]` を `series[0]` へ注入する（`injectGDDMarks`）。go-echarts の `MarkLineData`/`MarkPointData` も大文字 JSON タグ非準拠ゆえ markArea と同型に map 注入する。
>   - **罠①: データ範囲外の markLine/markPoint は軸 max 未指定だと dataZoom 下で見切れる（実描画ゼロ）.** 目標線 `y=target`（target＞最終累積）・予測マーク `x=forecastDay`（forecastDay＞最終経過日）は**データ域外**ゆえ、`dataZoom` がデータ域のみを表示する初期状態では描画域外（y軸は最終累積まで・x軸は最終経過日まで）に落ちて**全く見えない**。→ **`xAxis.Max`/`yAxis.Max` をマーク位置＋余白で明示**してフレームに収める（`gddAxisMax`＝yMax=max(target,累積)×1.08・xMax=max(最終日,予測日)×1.05）。option JSON にマークは在る（§56.1 テストは緑）が**実機では不可視**で、ブラウザ実機スモークでしか露見しない（テストガイダンス集 §5.6.AA (l)）。
>   - **罠②: 共有 `EChartsInitializer` の `endLabel`/`sampling` がスカラ前提で座標ペアに対し壊れる.** App.templ の初期化は `option.series[0].endLabel.formatter = function(p){return p.value+unit}` を**全 `[data-echarts]` へ無条件付与**する（§1291）。value 軸の系列は `p.value` が `[x,y]` **配列**ゆえ `[59,1013.875] + "℃·日"` が配列を文字列化して **"59,1013.875℃·日" のゴミラベル**を曲線端に出す（`lttb` sampling も時系列前提で value 軸に不適）。→ **`endLabel`/`sampling` 付与を `!el.hasAttribute('data-no-connect')` でゲート**し、value 軸チャート（後述の connect 除外マーカーを流用）には付与しない。既存4チャート（スカラ category 軸）は従来どおり付与＝後方互換。
>   - **connect も除外（`data-no-connect`）.** 経過日数 value 軸は時刻 category 軸の `echarts.connect`（カーソル連動）と意味が合わない。コンテナに `data-no-connect` を付け、`initScope` の connect 収集のみ `!el.hasAttribute('data-no-connect')` で除外する（init/dispose は全 `[data-echarts]` のまま＝後方互換）。この属性が罠②のゲートも兼ねる。実機検証は `getInstanceByDom(el).group`（value 軸チャートは `null`＝非連動）で固定（テストガイダンス集 §5.6.AA (l)）。
>
> - **heatmap/カレンダー/visualMap はネイティブ型で組め、inject は markLine/markArea 端点だけ（heat-stress-thi・2026-06-30）.** 高温ストレス（THI 時間帯×日 heatmap・熱帯夜 calendar・visualMap）は本リポジトリ初の非 line/bar 可視化だが、`charts.NewHeatMap()`＋`opts.Calendar`＋`opts.VisualMap{Type:"continuous", InRange:&opts.VisualMapInRange{Color:[…]}}` の **go-echarts ネイティブ型**で表現でき、`base.go` の独自 MarshalJSON が `calendar`/`visualMap`/`coordinateSystem`/`inRange` を正しい camelCase で吐くため §1288/§1297 の小文字キー自前注入は**不要**（標準の `json.Marshal(chart.JSON())` 経路を通すだけ）。inject が要るのは依然 markLine/markArea/markPoint の端点キー（`yAxis`/`xAxis`/`coord`）のみ（heat ではトレンド棒の Sen 傾き markLine だけ §1297 同型で注入）。
>   - **calendar 束縛の手順**: `hm.AddCalendar(&opts.Calendar{Range:[…], Orient:"horizontal"})`（`AddCalendar` が内部で `hasXYAxis=false` に切替＝**直交 xAxis/yAxis を出さない**）＋ `hm.AddSeries(name, data, charts.WithCoordinateSystem("calendar"), charts.WithCalendarIndex(0))`。heatmap データは hour×day が `opts.HeatMapData{Value:[]any{hour, dayIdx, value}}`、calendar が `[]any{"YYYY-MM-DD", value}`。y 軸（日ラベル）は `charts.WithYAxisOpts(opts.YAxis{Type:"category", Data:labels})`、x 軸（時刻）は `SetXAxis([]string{"0"…"23"})`。
>   - **これらも `data-no-connect` 必須（§1300 の一般化）.** calendar/heatmap は visualMap 連続色で**時刻 category 軸を持たない**ため `echarts.connect`（カーソル連動）と非互換。GDD（value 軸）と同じく全 heat コンテナに `data-no-connect` を付け、温湿度 line の connect 群に混ぜない（実機で `getInstanceByDom(el).group===null` を確認・テストガイダンス集 §61.4 同型）。
>   - **period 非連動パネルの配置（GDD と同型）.** calendar=直近1年固定・年スケールゆえ期間セレクタに連動させない。`buildChartArea`（Show と期間切替 Chart の**両方**が呼ぶ）では組まず、**Show 経路のみ**で組んで `#device-chart-area` フラグメントの**外**（ページ直下の兄弟 `<section>`）に描く。DTO は `DeviceChartAreaView` 末尾に持てるが（末尾非破壊追加）、フラグメント templ は描画せず**ページが描画**する＝期間切替の部分更新で再計算・再描画されない（統合テストで「フルページに在る／Chart フラグメントに不在」を固定）。
>   - **暑熱=暖色など visualMap の向き.** `inRange.color` を低→高で薄→濃暖色（例 `["#fff5b1","#ffa94d","#f03e3e","--color-heat"]`・モック凡例 `.heat-scale-bar` グラデと一致）にする。色配列順までは option JSON テストで固定できるが、**高値が濃端へ向くか（min/max 軸割当）の概念反転は実機スモークでしか捕捉できない**（[[project_vpd_physics_convention]]・テストガイダンス集 §56.1/§5.6.AA(i)）。
>
> - **計算系列（集計平均・回帰線・CI 帯 等）の表示値は handler 側で事前丸めする（seasonal-trend・2026-06-29）.** 共有 `EChartsInitializer` の整形は**場所により非対称**で、`tooltip.formatter` は数値を `v.toFixed(2)` で丸めるが、**`endLabel.formatter`（`p.value + unit`）と `markPoint`（max/min ラベル）は系列値を生のまま表示する**。これは §1299 の「value 軸＝`[x,y]` 配列が壊れる」とは**別の罠**で、こちらは**スカラだが割り切れない長小数**が原因。生センサー値は `numeric(5,2)` で元々2桁ゆえ顕在化しないが、**読み取り時に計算した系列**（ロールアップ平均・Sen トレンド線・移動平均・CI）は `26.79310345℃` のような長小数が endLabel/markPoint に出て、**サマリ表（handler で `%.2f` 整形済み）と不整合**になりクラッタする。→ handler の chart spec 構築時に**表示専用の丸め**を施す（`internal/handler/seasonal_trend_handler.go` の `chartRound2()`＝`math.Round(v*100)/100` の入力非破壊スライス。`RollupAvg`/`SenLine`/`CILower`/`CIUpper`/`Climatology` に適用）。**検定計算（MK/Sen 等）は丸め前の生値で済ませ、丸めるのは描画系列のみ**（数値正確性を損なわない）。共有 `App.templ` の endLabel formatter は**触らない**（全チャート波及・境界外）＝**各 handler が自分の計算系列を丸める**のが定石。テストは option JSON に長小数が出ないことを §56.1 でアサート可（例 `strings.Contains(option,".793103")==false`）だが、**「見た目の煩雑さ」自体は option に値が在るため §56.1 では出ず**、ブラウザ実機スモーク（テストガイダンス集 §5.6.AA）でしか視認できない（GO 判定後の chrome-devtools スモークで発見した）。
...
> **select 候補の供給は2方式。固定 enum はループ生成（上記）、件数多・加工ラベル・DB 由来はハンドラが組む 📅 2026-06-26 — device-location-select で確立**:
> - **方式b（templ が domain 列挙を直接 range）**: 上の `domain.AllMetrics()` ループ。固定 enum（`metric`/`operator` 等・少数）向き。選択復元は `selected?={ v.X == string(x) }` の文字列比較。`view → domain の表示メソッド（Label()）`参照は許容（表示専用メソッドのみ）。
> - **方式a（handler が `[]SelectOption{Value,Label,Selected}` を組み View 経由で渡す）**: 件数が多い／表示ラベルに加工が要る（同名区別「具志川（うるま市）」等）／DB 由来で件数可変（`device_id` 等）向き。templ は `v.Localities` を range するだけで domain を直接参照しない。選択復元は**handler が `Selected` を立てる**（device-location-select の地域 select＝`component.SelectOption` ＋ `localityOptions(selected)`）。
>
> 使い分けの目安: **少数の固定 enum＝方式b／多件数・加工ラベル・DB 由来＝方式a**。どちらも `oneof`・DB CHECK・select の許容集合は単一ソース（domain）に保つ。domain の取得・分岐ロジックは handler に置き、view には整形済み値だけ渡す。

> **検索可能 select（Tom Select）は `value`＝正規キー／`label`＝認識名にすると、既定の検索だけで「別名検索」も成立する 📅 2026-06-27 — device-location-select で確認**: Tom Select は `searchField` を明示しなければ option の**表示テキスト（label）**を検索対象にする（App.templ の `initTomSelect` は `searchField` 未設定＝既定）。そこで `value` に正規キー（DB 格納値・`oneof`/CHECK の許容値。例「具志川市」「具志川村」）、`label` に補助情報を併記した認識名（例「具志川（うるま市）」「具志川（久米島町）」）を入れると、追加設定なしで次が同時に成立する:
> - **同名区別**: 表示が「短縮名（現市町村）」で重複しないため、利用者が取り違えない。
> - **別名（エイリアス）検索**: label に旧名と現市町村名の両方が含まれるため、「具志川」でも「うるま市」でも候補が絞り込まれる（「佐敷」でも「南城」でも可）。`searchField` の拡張も hidden の別フィールドも不要。
>
> 送信されるのは常に `value`（正規キー）で、表示・検索だけが `label`。**注意**: 同名の**短縮名**（例「具志川」単独）は `label` 上にしか現れず一意でないため、サーバ側で文字列→キーを解決する経路（CSV 取込・既存値の backfill 等）では短縮名を解決キーにしない（曖昧）。一意に解決できるのは `value`（正式名）と、重複しない短縮名のみ（`domain.ParseLocality` はこの方針）。
...
>
> **⚠️ 適用範囲（swap される select だけが対象）📅 2026-06-26**: 本節（TS-1 等）と C12 の `htmx:beforeSwap`/`afterSwap` による**再初期化が要るのは、その `<select>` 自身またはそれを含む領域が HTMX で swap される場合**（カスケード連動・インライン CRUD・部分更新）に限る。**フルページ遷移の通常フォーム内の Tom Select（例 device-create / device-edit の地域 select。§6 のフルページ POST）は swap されないので、`App.templ` の初回 `initTomSelect()` だけで足り、swap ライフサイクルの配線は不要**。単一 select を「念のため」swap ライフサイクルに乗せる過剰実装を避ける（device-location-select は単一 select・カスケード無しのため `App.templ` を改変していない）。
...
### TS-13: CDN tom-select.css が本 CSS より後読み込みのため `.ts-control`/`.ts-dropdown` の色指定が後勝ちで負ける 📅 2026-07-03 — dark-mode で確立

**症状:** `.form-group .ts-wrapper.single .ts-control { background: var(--color-surface); ... }` のように自前 CSS で Tom Select コントロールの配色を上書きしているのに、実機では反映されず CDN 既定（白背景・濃色文字固定）のまま残る。ライトテーマでは偶然近い見た目になるため気付かれにくく、ダークテーマ導入など配色を大きく変える変更で初めて顕在化する。

**原因:** `App.templ` は自前 `style.css` の `<link>` を CDN `tom-select.css` の `<link>` より**前**に配置している（キャッシュバスティング対象の自前 CSS を先に、外部 CDN を後に読み込む一般的な順）。CSS の詳細度が同じ場合、**後から読み込まれたスタイルシートのルールが勝つ**（ソース順のカスケード）。`.ts-dropdown`（body 直下に生成されるドロップダウン）は既に TS02（z-index 上書き）でこの問題への対策が入っていたが、`.ts-control`（`<select>` の可視コントロール本体）側は同じ問題を抱えたまま見過ごされていた。

**対策:** `.ts-dropdown` の TS02 と同じパターンで、CDN と衝突する色プロパティに `!important` を明示する。

```css
/* CDN tom-select.css は本CSSより後に読み込まれるため (App.templ)、background/color/border-color は
   .ts-dropdown と同様に後勝ちでCDN既定(白背景・濃色文字固定)が勝ってしまう。ライトでは偶然
   同系統の見た目になり気付きにくいが、配色変更(ダークテーマ等)で顕在化するため !important で明示上書きする。 */
...
**発見経路:** ダークモード実装のテーマ patch を Go テスト（構造アサート）で固定した後、実機ブラウザで `getComputedStyle()` を用いて実際に適用されているスタイルを確認したところ、CSS 定義上は正しいのに描画結果が食い違っていることが判明した。**「CSS ソースに正しい宣言があること」と「実際にその宣言が勝っていること」は別問題**であり、CDN 等の外部スタイルシートと同じセレクタ/プロパティを扱う箇所は、Go の構造アサートだけでなく実ブラウザでの `getComputedStyle()` 確認（または少なくとも `!important` 要否の見積り）を行うこと。

**影響範囲:** 自前 CSS が外部 CDN の CSS より**後**に読み込まれる構成であれば発生しない。本プロジェクトは `<link>` 順が「自前 CSS → CDN CSS」のため、Tom Select が上書きしたい全プロパティが対象になりうる。新規に Tom Select の配色を上書きする際は、まず `!important` 無しで実装し、`getComputedStyle()` で反映されているか確認する運用を推奨する。

**関連:** TS02（`.ts-dropdown` の z-index 上書き）、§40-B（CSS キャッシュバスティング・読み込み順）
...
#### 31.2 器（要素種別）の変更はモック正本へ先行反映。select の `<option>` は代表サブセットで可 📅 2026-06-26 — device-location-select で確立

§31／§31.1 は「クラス語彙の統一」「動的文字列は写経対象外」を扱うが、**要素種別そのものの変更（text input → select 等＝器の変更）** の扱いを補足する:

- **器の変更はモック正本（`mocks/html/{画面}.html`）へ先に反映してから templ を写経**する（モックが正本・§40-B 単一ソース・`HTMLモック作成ルール.md`）。例: 設置場所の `<input type="text" name="location">` を `<select name="locality" class="js-tom-select">` へ替えるなら、まず `device-create.html`／`device-edit.html` を select に書き換え、その後 `DeviceForm.templ` を写経する。**CSS（`style.css`）が変わらなければ `make sync-css` は不要**（既存クラスのみ使用）。
- **select の `<option>` 群はモックでは全件列挙しない**。モックは設計・CSS 確認用なので**代表サブセット**（空 option ＋ 数件。同名区別を見せるなら「具志川（うるま市）」「具志川（久米島町）」のような代表値を含める）で足りる。本番の全選択肢は templ 側で domain 列挙（方式b）または handler 供給（方式a・§2「select 候補の供給は2方式」）から生成する。モックの option 件数と本番件数は一致しなくてよい（モック反映義務は「器＝要素種別・クラス・配置」まで）。

#### 31.3 checkbox 群を `.form-group` に足すときは入れ子 label をラッパ div で囲って横並びにする 📅 2026-06-28 — sensor-data-export 項目フィルタで確立

§31.2 と同じ「器（要素種別）の変更はモック正本へ先行反映」だが、**checkbox/radio 群**を既存の `.filter-form`／`.form-group` に足すときの CSS レイアウト罠を補足する:

- `.form-group > label { display: block }`（フィールド見出し用スタイル）は**子結合子 `>` ＝直下の `<label>` だけ**に効く。checkbox の各 `<label><input>…</label>` を `.form-group` の**直下**に並べると、それぞれが見出し扱いで `display:block` になり**縦積み**になってしまう。
- 横並びにするには、checkbox 群を**クラス無しのラッパ `<div>` で囲み**、その中に入れ子 `<label>` を置く。入れ子 label は `.form-group > label` の対象外＝inline 既定のまま並ぶ。**独自クラスは新設しない**（§31・既存 `.form-group` 流用のため `make sync-css` 不要）。

```html
<div class="form-group">
  <label>出力項目（CSV）</label>            <!-- 直下 label = block（フィールド見出し） -->
  <div>                                     <!-- クラス無しラッパ（直下でないので block 化されない） -->
    <label><input type="checkbox" name="items" value="temperature" checked> 温度</label>
    <label><input type="checkbox" name="items" value="humidity" checked> 湿度</label>
  </div>
</div>
```

- templ での checked echo は**属性内 if**で書く: `<input type="checkbox" … if 含む判定 { checked }/>`。「適用済み選択を checked で復元」「未選択は全選択を既定」のロジックは **handler 側の単一関数に集約**し、出力列の選択・ダウンロードリンクの項目・フォーム echo の3出口で共有して食い違いを防ぐ（実装 `internal/handler/readings_export.go` の `effectiveMetricItems`）。GET 絞り込みフォームは結果領域（swap 対象）の**外**に置く（§3.5・swap 時に入力状態を保持）。

### 31.4 既存語彙に無い「状態表示クラス（信号色バッジ）」を足すとき — `@layer components` の variant ＋ domain列挙メソッドで配線する 📅 2026-06-29 — data-quality-meta で確立

§31 は「独自クラスを新設しない」が原則だが、**モックの既存語彙に無い状態表示**（品質レベルの信号色バッジ＝信頼/注意/不良 等）が要るときの正当な追加運用。実際 `.badge` はモック未定義だった（状態色は `.status-active`/`.status-inactive` の文字色のみ）。バッジは「部品の状態色」なので **utilities ではなく `@layer components`** に置く。

- **手順（モック正本先行・§31.2 と同じ精神）**: ①正本 `mocks/html/style.css` の **`@layer components`** に `.badge` 基底＋信号色 variant（`.badge-good`/`.badge-caution`/`.badge-bad`）を、**既存トークン**（`--color-primary`/`--color-warning`/`--color-danger`）流用で追記。②モック HTML（`readings.html` 等）に器（バッジ・フラグ列・メトリクスボックス）を写経。③`make sync-css` で本番へ反映（§40-B 単一ソース・§5189 追記運用＝本番ファイルに直接書かない）。④templ を写経。
- **状態↔色の対応は domain列挙のメソッドで持つ**（view→domain 表示メソッド）。`type QualityLevel string` に `BadgeClass()`（`Metric`/`Crop` と同じ表示メソッド書式）を置き、templ は `class={ "badge", level.BadgeClass() }` で**スペース結合**する。状態追加・色変更が domain 1 箇所に閉じ、CSS variant と **1:1 対応**する。DB 非永続の計算由来カテゴリゆえ §100 の VARCHAR+CHECK は不要＝純 Go 列挙。
- **`BadgeClass()` は dot 無しのクラス名を返す**（`"badge-good"`）。設計書がセレクタ記法 `.badge-good` と書いていても、`class` 属性に乗る値は **dot 無し**。dot 付きを返すと `class="badge .badge-good"` になり効かない（CSS セレクタと class 属性値を混同しない）。
- **出し分け**: 総合バッジ（信号色）は状態がある時だけ出す（`if v.Quality.HasData { <span class={ "badge", v.Quality.Level.BadgeClass() }>…</span> }`）。行レベルのタグ（フラグ）は基底 `.badge` のみ（variant なし）で中立に並べ、総合バッジだけ信号色 variant を付ける。「異常行のみ／正常行は空」と「HasData=false で信号色を出さない」のテスト固定はテストガイダンス集 §60.4。
- **グラフ内部の動的描画は器のみ反映**: 欠測の線分断・markArea（go-echarts 動的描画）は**モック反映対象外**で、凡例/注記/バッジ/フラグ列/メトリクスボックスの**器のみ**をモックへ反映する（`feedback_mock_graph_rendering_exception`・§31.2 の「器の変更はモック先行」と同じ精神だが、描画自体は例外）。
...
## 66. テーマ切替（ダークモード等）実装時の CSS/JS 設計パターンと落とし穴 📅 2026-07-03 — dark-mode で確立

> **cc-sdd への価値:** ライト/ダークの2値切替に限らず、将来的なテーマ・配色機構（コントラストモード・ブランドテーマ切替等）を実装する際に再利用できる。CSS トークン設計・サードパーティ CSS/JS ライブラリとの統合・実測ベースの検証手法をまとめる。

### 66.1 ダーク2導線のトークン再マップは「パレット層（生値）+ セマンティック層（役割名）」で単一ソース化する

OS 追従（`@media (prefers-color-scheme: dark)`）と手動選択（`[data-theme="dark"]` 属性）の**2つの導線**を同時にサポートする場合、ダークの生値（hex 等）を2箇所に書くと値がズレる危険がある。

```css
:root {
  /* ライト値（現行）はそのまま。ダーク生値はここで一度だけ定義（パレット層） */
  --color-surface: #ffffff;
  --palette-dark-surface: #1c1f24;
}
/* 両導線は同じパレット変数を参照する（セマンティック層の再マップ） */
@media (prefers-color-scheme: dark) {
  :root:not([data-theme="light"]) { --color-surface: var(--palette-dark-surface); }
}
:root[data-theme="dark"] { --color-surface: var(--palette-dark-surface); }
```

`:root:not([data-theme="light"])` を `@media` 側に付けるのがポイント：OS がダークでも、利用者が明示的にライトを選んでいれば（`data-theme="light"`）このブロックを無効化できる。手動選択が無い（`data-theme` 属性が無い）ときだけ OS 設定に従う、という優先順位を CSS だけで表現できる。

**テスト化**: 両導線ブロックが再マップするカスタムプロパティ名の**集合が完全一致**することを、正本 CSS を構造的にパースして機械検査する（テストガイダンス集 §62.1）。片方だけ更新して不整合になる回帰を機械的に防げる。

### 66.2 SVG アイコンを CSS トークンでテーマ追従させる — `mask-image` を複合要素へ直接適用すると要素全体が消える

「固定色でエンコードした SVG data URI を `background-image` に使っているため、アイコンの色を CSS 変数で動的に変えられない」という問題に対し、**`mask-image` + `background-color` の組み合わせ**（SVG の形状をマスクとして使い、実際の色は `background-color` で与える手法）が一般に紹介されるが、これは **「要素の全体がそのアイコン専用（他に枠線・文字・背景を持たない）」場合にのみ安全**。

`<select>` のような「枠線・背景・文字を持つ複合要素」に `mask-image` を直接適用すると、CSS の `mask` はその要素の**描画結果全体**（背景・枠線・テキストを含む）に対してマスクをかけるため、指定した小さなアイコン領域**以外が丸ごと透明になる**（枠線もテキストも消える）。実機で `mask-image` を `<select>` に直接適用したところ、要素全体が「ごく小さいアイコン片以外は完全に消える」現象を確認した。

**安全な代替**: エンコード済み SVG の色部分だけをテーマごとに差し替えた**2種類の `background-image` 値**を、それぞれ CSS カスタムプロパティとして保持し、テーマ導線側で参照先を切り替える。

```css
:root {
  --select-arrow: url("data:image/svg+xml;...stroke='%236c757d'...");  /* ライト用 stroke 色 */
}
:root[data-theme="dark"] {
  --select-arrow: url("data:image/svg+xml;...stroke='%239aa0a6'...");  /* ダーク用 stroke 色 */
}
.form-group select { background-image: var(--select-arrow); }
```

`mask-image` の「専用の小要素になら安全（アイコン専用の `<span>` 等）」という前提条件を必ず確認し、複合要素（フォームコントロール等）に適用する前に実機で検証すること。

### 66.3 サードパーティ JS 描画ライブラリ（ECharts 等）のクローム色をクライアント側でテーマ追従させる設計

サーバ側で生成する option/config に固定色を持たせず「クローム色（軸・凡例・ツールチップ等の UI 装飾色）中立」に保ち、クライアント側 JS が現在テーマに応じた色を後から `setOption` 等でマージする設計にすると、テーマ追加時にサーバ側の再実装が不要になる。

- **既存インスタンスの `dispose→re-init` を避け、`setOption` の部分マージで色だけ更新する。** これによりズーム位置・凡例のトグル状態等のユーザー操作状態が保持される。
- **ライブラリの「値を取得する」API（`getOption()` 等）と「初期構築時に渡す」生 JSON は形状が異なりうる**。ECharts の場合、`getOption()` は `xAxis`/`yAxis`/`legend`/`tooltip`/`visualMap`/`dataZoom`/`calendar` 等を**常に配列**へ正規化して返すが、サーバから届く生成直後の option JSON はコンポーネント数に応じて単一オブジェクトのままのことがある（1本の軸なら `xAxis: {...}`、2本なら `xAxis: [{...},{...}]` 等）。クライアント側でパッチを組み立てるヘルパは、**両方の形状を吸収する**（`Array.isArray` で判定し、配列なら同数複製、単一なら単一のまま返す）汎用処理にすること。
- **往復切替（テーマA→B→Aと戻す）を成立させるには、両テーマとも明示値を返す**（マージ方式では「未指定に戻す」ことができないため、「解除」ではなく「明示的にライト値を書き戻す」必要がある）。

### 66.4 サードパーティライブラリの「既定値」は推測せず、実ブラウザで実測してから実装する

ライブラリのドキュメントや記憶に頼って「既定はこの色のはず」と決め打ちすると、実際のバンドルバージョンでズレていることがある（マイナーバージョン間で既定値が変わっている場合や、記憶違いの場合）。

**実測手順**: 実際に self-host しているバンドル済み JS ファイルを使い、chrome-devtools 等でミニマルな HTML を開いて実際にライブラリを初期化し、`getOption()`（や同等の状態取得 API）で**実測した値**をそのままライトテーマの明示値として採用する。これにより「ライトテーマは視覚差ゼロ（既定同値）」という無回帰要件を正確に満たせる。

```javascript
// 例: ECharts の dataZoom(slider)/calendar の実際の既定値を実測する
var inst = echarts.init(document.createElement('div'));
inst.setOption({ /* 最小構成 */ });
console.log(JSON.stringify(inst.getOption().dataZoom[1]));  // 実測した既定値をコードへ転記
```

**関連:** テストガイダンス集 §62（dark-mode で確立したテスト知見）
