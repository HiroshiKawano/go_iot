...
| 期間切替（24h/7d/30d） | `true` | 期間設定をURLに反映する |
...
>
> `onChange` で `change` イベントを明示的に発火する理由: HTMX の `hx-trigger="change"` が Tom Select 適用済みのフィルタ用 `<select>`（デバイス絞り込み・計測種別フィルタ等）で確実に動作するようにするため（詳細は§4参照）。ネイティブ select（R22-2限定列挙対象: boolean型・表示件数・期間 24h/7d/30d 等）はブラウザが `change` イベントを直接発火するため本対応は不要。
...
> **R22-2 ネイティブ select の場合:** ネイティブ select（R22-2: boolean型・表示件数・期間 24h/7d/30d 等）は独自UIを構築しないため、Alpine.js の `x-model` が直接機能する。以下の `onChange` 手動同期パターンは Tom Select 適用 select 専用であり、ネイティブ select には不要。
>
> ```html
...
| | `device-chart-area` | 温度グラフ + 湿度グラフ + 期間セレクタを内包するラッパー | 期間選択（24h/7d/30d）切替の `hx-target`。この領域全体を差し替え、期間ボタンの active 状態もサーバー側往復（§4 device-show / §10-D） |
...
| | `alert-rule-list` | ルール一覧テーブルの `tbody`（`.rule-list` / `.data-table` 内） | ルール削除後に一覧を差し替え |
...
| URL | id | 対象要素 | 用途（HTMX） |
|-----|----|---------|-------------|
| `/alerts/history` | `alert-history-list` | 履歴一覧テーブルの `tbody`（`.data-table` 内） | フィルタ検索（デバイス・期間）・ページ送り時に履歴行を差し替え。`thead` は固定 |
| | `alert-history-pagination` | `.pagination`（ページネーションのラッパー） | ページ送り時にページ番号リンクを更新（`alert-history-list` と同時に OOB Swap） |
| | `alert-history-filter` | フィルタフォーム（`.filter-form`） | デバイス選択肢の動的更新等が必要な場合のターゲット（通常はフォーム自体は固定でよい） |

> ※ フィルタフォーム（`.filter-form`）は `method="GET"`（R17）。デバイス選択（`.js-tom-select`）は Tom Select 適用。`hx-get` + `hx-target="#alert-history-list"` で履歴を、`hx-swap-oob` で `alert-history-pagination` を更新する。20件/ページ（画面設計書(静的).md）。
...
| `alert-rule-list` | alert-rules | `hx-target`（tbody、削除時） |
| `alert-rule-row-{rule}` | alert-rules | `outerHTML`（有効切替の行） |
| `alert-history-list` | alert-history | `hx-target`（tbody） |
| `alert-history-pagination` | alert-history | OOB |
| `alert-history-filter` | alert-history | `hx-target`（任意） |
...
> デバイス情報パネル + 期間切替（24h/7d/30d）+ 温度グラフ + 湿度グラフ + 最新計測データ（10件固定）で構成。**期間切替とグラフ差し替えが本画面の HTMX 中核。**

| 操作 | 方式 | メソッド | URL | ターゲット | swap | トリガー | レスポンス |
|------|------|---------|-----|----------|------|---------|----------|
| 初期表示 | フルページ | GET | `/devices/{device}` | — | — | — | `DeviceShow.templ`（デフォルト期間 24h でグラフ + 最新計測10件） |
| 期間切替（24h / 7d / 30d） | HTMX | GET | `/devices/{device}/chart?period={24h\|7d\|30d}` | `#device-chart-area` | innerHTML | click | グラフ領域 Fragment（温度グラフ + 湿度グラフ SVG を再生成）+ 期間ボタンの active 状態 |
...
- 24h は `sensor_readings` 生データ、7d/30d は日次集計クエリ（`GROUP BY DATE(recorded_at)`、DB設計書.md）を sqlc クエリで取得する。
...
|------|-----------------|------------|
| 計測値一覧 `/devices/{device}/readings` | センサー計測（reading） | `#device-readings-list` |
| アラート履歴 `/alerts/history` | アラート通知（alert） | `#alert-history-list` |
...
> 自前トークン（`var(--space-4)` 等）や `@layer utilities` のユーティリティクラス（`.u-mb-4` 等）でスペーシングや配色を委譲する場合も、上記の構造クラス（`.card` `.form-group` 等）は維持し、ユーティリティを併用する。CSS方針は .kiro/steering/tech.md 参照。
...
> **cc-sdd への価値:**
> デバイス詳細画面（`/devices/{device}`）の計測値グラフで期間（24h / 7d / 30d）を切り替える際の Handler 設計・HTMX 統合パターン。
...
| 項目 | 判断基準 |
|------|---------|
| サーバー側 vs クライアント側 | 同一データセットの単純な表示切替ならクライアント側（Alpine.js）でも可。期間ごとに異なる集計クエリ（24h/7d/30d で粒度が変わる）が必要ならサーバー側 |
...
- **§64**（HTMLコメント内の `<x-...>` が Blade コンポーネントとしてパースされる）— templ にこの問題はない
