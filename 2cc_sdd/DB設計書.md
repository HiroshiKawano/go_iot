...
- **認証**: ESP8266 は自作 Bearer トークンミドルウェア + `device_tokens` テーブル、ブラウザは Session（`alexedwards/scs` + PostgreSQL セッションストア）
...
### SensorReading 用（ESP8266 からの API 受信）
...
| フロー | 関連テーブル |
|--------|-------------|
| ESP8266 → API → DB保存 | sensor_readings, devices (last_communicated_at 更新) |
...
```
ESP8266 → POST /api/sensor-data → Handler（同期処理）
