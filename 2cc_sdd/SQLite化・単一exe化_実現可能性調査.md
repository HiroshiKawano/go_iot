...
1. **温湿度の NUMERIC 精度**: REAL（float64往復・改修小）で良いか、厳密10進が要件か（閾値比較の正確性に直結）
2. **日次バケットの TZ**: `date(recorded_at,'+9 hours')` で JST 境界に補正するか、UTC据置きを許容するか
3. **デスクトップ UX のスコープ**: 初版は「ブラウザ自動オープン」のみで十分か、systray常駐やコンソール窓非表示（`-H windowsgui`＋ログのファイル出力）まで含めるか
