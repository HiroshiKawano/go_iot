...
| **E2** | `dark-mode` | 全画面横断の **ダークモード**（既定OS追従＋手動トグル＋localStorage永続・`data-theme`）。CSSトークン2層化（palette/semantic）＋`color-scheme`＋Tom Select `.ts-dropdown`上書き＋**ECharts 12コンテナのクローム patch**（軸/凡例/visualMap/dataZoom・init時＋themechange時の2導線）。トークン誤用バグ2件（`.error-toast`/`.badge-caution`）同時修正。**スキーマ非変更・データ意味色は両テーマ据置** | 全9画面＋login/register・正本CSS・App/Guest レイアウト・EChartsInitializer | S1, E1（全画面実装済み） | [session-10](spec-init-prompts/session-10-dark-mode.md) |
...
- **E2** — 白一色の現UIにダークテーマを追加する UI/UX 横断拡張。事前調査は [ダークモード調査.md](ダークモード調査.md)（v2.1・全項目裏取り済み）が権威：生色インベントリ・トークン誤用2件・Tom Select CDN の白ハードコード・ECharts クローム（visualMap/dataZoom 含む）・markArea 注入色6種の扱いまで確定済み。製品判断3点は 2026-07-02 ユーザー確定（保存先=localStorageのみ／初回=OS追従／**Guest画面にもトグル設置**）。トグルは共通 `ThemeToggle.templ` を App ヘッダーと Guest 独自配置で共有。モック単一ソース規約に従いトグル要素・ダークブロックはモックHTML＋正本CSSへ先行反映。ライトの見た目は無回帰。
