...
| 残課題 | 区分 | 状態 / 備考 |
|---|---|---|
| **実センサー値の取得**（ESP8266 + 温湿度センサー） | デバイス | ⛔ 未着手。當山さん検証は固定値 `value=123` の HTTPS GET のみ |
| **実センサー値の送信**（自前 Go API 宛） | デバイス | ⛔ 未着手。送信先・形式を PHP テスト用 GET から **Go API の JSON POST** へ作り替える必要（→ 8-2） |
...
> 2. **優先2（固定値を Go API へ）:** GET ?value= をやめ、**固定の JSON を `POST /api/sensor-data` へ Bearer 付き**で送り、`201` と DB 1行・画面反映を確認。
...
> ※ これは骨格であり本リポジトリには未コミット。実装する場合はファーム用の別リポジトリ/ディレクトリに置く（本リポジトリのスコープ外＝[steering/product.md](../.kiro/steering/product.md)「スコープ外」）。
