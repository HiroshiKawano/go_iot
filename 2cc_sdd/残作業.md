...
| 4 | **A-3** | バイナリのビルドと配布・DBマイグレーション・systemd 起動・日次バックアップ仕込み | AI（SSH） |
...
>
> **実行主体**: cron は **SSH オペレーター `<user>`** の crontab で回す（C-2-3 の確認コマンド `crontab -l` が sudo 無しで `<user>` の crontab を見る前提と一致）。`pg_dump` のパスワードは `<user>` の `~/.pgpass`（600）で解決し、コマンドラインや crontab に平文を置かない。

**▶ AIへの依頼（プロンプト例）**
> 「go-iot-prod に DB の日次バックアップを仕込んで。`pg_dump` を `/var/backups/go_iot` に gzip で吐く `/usr/local/bin/pg_backup.sh` を置いて、毎日 03:10 に走る cron を `<user>` に登録。パスワードは `~/.pgpass`（600）で解決して平文を crontab に書かないで。14 日より古いダンプは自動削除。仕込んだら手動で 1 本取って、復元検証用 DB に流して戻せることまで確認して。」

**🤖 AI が行う操作（ssh 経由。`<DBユーザー>`=`go_iot`、`<本番強パスワード>`=STEP 4 の `DATABASE_URL` と同一値）**
```bash
ssh <user>@<静的IP> 'set -e
  # ① 保存先とログを <user> 書込可で用意（pg_dump は <user> として走る）
  sudo mkdir -p /var/backups/go_iot && sudo chown '"<user>"':'"<user>"' /var/backups/go_iot
  sudo touch /var/log/pg_backup.log && sudo chown '"<user>"':'"<user>"' /var/log/pg_backup.log

  # ② パスワードは ~/.pgpass(600) で解決（host:port:db:user:password）。crontab に平文を置かない
  umask 077
  printf "localhost:5432:go_iot:%s:%s\n" "<DBユーザー>" "<本番強パスワード>" > ~/.pgpass
  chmod 600 ~/.pgpass
...
EOF
  ...
  # ④ cron を <user> に冪等登録（毎日 03:10。重複登録を避けて差し替え）
...
> ⚠️ **`<DBユーザー>`/`<本番強パスワード>` は STEP 4 の `DATABASE_URL` と必ず一致させる**（不一致だと `pg_dump: error: ... password authentication failed`）。パスワードはチャットに丸出しせず、`~/.pgpass` は 600・`<user>` 所有を厳守。`pg_dump` は同梱の PostgreSQL クライアント（`postgresql-${PG_VER}`）に含まれるため追加導入は不要。

**🙋 人が確認/承認する点**
- `~/.pgpass` が **600・`<user>` 所有**で、パスワードが crontab・スクリプト本文・チャットログに**平文で出ていない**こと（出ていたらローテーション）。
...
ssh <user>@<静的IP> "createdb -h localhost go_iot_restore_test 2>/dev/null || true; \
...
- **【決定済み】ブラウザ MCP（chrome-devtools）で AWS 画面を AI に運転させるか**: **採用**。①のサインアップと S-3-1 の IAM コンソール操作を chrome-devtools MCP で AI に代行させる（非機微フィールド・クリックのみ）。**制御は人が握る MFA 認証**（任せたくない時は認証しない＝AI は admin/root セッションを持てない）＋**機微入力（カード/各種コード/パスワード/MFA/CAPTCHA）は AI に渡さない**＋**ブートストラップ後は admin セッションをログアウトし最小権限 IAM に戻す**。admin 迂回は「一度きり・監督付き」に限定（詳細は §S 着手前ゲート①・§S-3-1 の 🤝 注記）。A-4 のアプリ Web UI 操作も同様に chrome-devtools MCP で自動化（下記「Web UI 操作の自動化」）。
...
| 実機基板 | 自作（LCD/ファン付きセンサー/スクリュー端子）・**USB-シリアルチップ非搭載**・**自動リセット非対応**（RTS→RST のみ、DTR→GPIO0 未配線）・自前 AC アダプタ給電 |

### F-1. MAC アドレス取得（スケッチ書込不要・esptool）

- **`esptool` は v5.3.0（pip venv）を使う**。arduino-cli 同梱の **esptool.py v3.0 は使わない**（F-7 参照）。
  ```bash
  python3 -m venv esptool-venv && esptool-venv/bin/pip install esptool   # wheel・コンパイル不要(brew formula はllvm/rust源ビルドで重い→不可)
  esptool-venv/bin/esptool --port /dev/cu.usbserial-XXXX --before no-reset --chip esp8266 read-mac
  ```
- この基板は自動リセットが効かないため、**手動で書込モードに入れてから** `--before no-reset` で読む（F-7）。MAC は `WiFi.macAddress()`＝efuse base MAC と一致。
...
### F-8. センサー検出のつまづき → I2C `0x45` 発見（核心）

- 初回起動ログ: **`[SHT31] not found at 0x44`**（センサー配線済みなのに検出されない）。WiFi/NTP/MAC は OK。
- **真因**: SHT31 の ADDR ピンが High → **I2C アドレスが 0x44 ではなく 0x45**（配線・ピンは想定どおり GPIO4/5）。
- **発見法**: **I2C スキャナ `firmware/i2c_scanner/`**（全ピン組合せ×全アドレス総当たり）を焼く → `SDA=GPIO4 SCL=GPIO5 で 0x45 応答（SHT3x）` を確認。
- **修正**: `config.h` の `SHT31_ADDR=0x45` → 再ビルド → 再書込（1行で解決）。
- 教訓: 引継ぎメモでセンサー型番は未確定だった（DHT/SHT31/BME280…）。**実機センサーは I2C スキャナで address/pin を実測確定**するのが確実（BME280=0x76/0x77, AHT=0x38, HTU/Si70xx=0x40 等）。

### F-9. 実機シリアルの採取（USB チップ非搭載の基板）

- 実機基板は USB-シリアル非搭載 → **外付け FTDI を UART に接続**: `FTDI GND↔基板 GND` / `FTDI RXD↔ESP TXD`（受信のみなら 2 本）。**FTDI VCC は繋がない**（基板は自前 AC アダプタ給電・2 電源衝突回避）。
- 接続で `/dev/cu.usbserial-XXXX` が復活 → pyserial 115200 で採取。**FTDI は監視用なので、確認後は抜いてよい**（基板は自前電源で稼働継続）。
