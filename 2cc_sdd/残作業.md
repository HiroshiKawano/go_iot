...
> **本書の既定は self-host 版（上の表）＝2026-06-23 実証済み**（付録E/§S-6-2）。下記★のマネージド版は本案件で**未検証の将来オプション**で、再現時は使わない。S-2〜S-6 の詳細（IAM 最小権限・安全策・read-only 疎通）は self-host 版の具体手順でもある。

#### （未検証・将来オプション）マネージド版 AWS MCP Server の接続（GA・AWS SSO）— 本案件では使わない
...
### S-8. （補足・未検証）マネージド版の利点とトレードオフ（本案件では採用しない）

**本案件では使わない**（self-host＝ルートB が実証ルート）。将来 SSO へ移行する際の検討材料として、利点／トレードオフだけ補足する（接続手順は S-1 の該当節を参照）。
