...
- **残（未了）**: デバイス登録 → `gen-token` → `cmd/sensor-sim` で HTTPS POST(201) → アラート発火 → グラフ反映（§A-4 step3-9）、§A-3 STEP8 日次 `pg_dump` cron。
...
- **SSH 再接続**: `aws lightsail get-instance-access-details --instance-name go-iot-prod --region ap-northeast-1 --profile go-iot-mcp` を再取得（証明書短命）→ 鍵/証明書を `~/.ssh/lightsail-goiot.pem`(+`-cert.pub`) に置き直す。
...
- ✅ **`esptool` v5.3.0（pip venv）で `write-flash`**:
  ...
  ```
- **リセット配線（この基板）**: **RTS→RST のみ・DTR→GPIO0 未配線**。だから自動リセットでは書込モードに入れない。
  - **書込モード（手動）**: GPIO0 ボタンを押したまま → RST ボタンを一度押して離す → GPIO0 を離す。
  - **書込後の起動**: `--after no-reset` で bootloader 待機 → **RST ボタンを物理押下**して新ファーム起動が確実（シリアル open 時の DTR/RTS 極性が不定で自動パルスは空振りしうる）。
...
*本書は 2026-06-23 時点。MCP サーバ/AWS 仕様/料金/承認ゲート挙動は実作業時に再検証・更新すること。A→B は実施・完了（付録E＝サーバ / 付録F＝実機デバイス）。本番 URL・device_id・トークンは B（[firmware/README.md](../firmware/README.md)）にも反映済み。残りは C（現地実証・継続運用）。*
