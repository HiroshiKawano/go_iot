...
本機能は、農場運営者が ESP8266 温湿度センサー（デバイス）を Web UI から**登録・編集**するフォーム画面を提供する。S1（web-foundation-auth）で確立した認証・レイアウト・ミドルウェア・所有者認可・CSS 配信の上に、新規登録（GET フォーム / POST 実行）と編集（GET フォーム / PUT 実行）を**同一フォーム構造**で実装し、バリデーション・入力値復元・MAC 一意性・成功時リダイレクトを完成させる。
