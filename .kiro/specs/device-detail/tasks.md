...
- [x] 6. 統合検証とカバレッジ
  - 主要ユーザーフローを通しで検証する: 期間切替で `#device-chart-area` のみ差し替わりテーブルが据え置かれること、期間がページ URL に反映されること、削除モーダル→確認→/dashboard 遷移、CSRF トークン往復（GET でトークン取得→DELETE 成功／トークン無し DELETE→419・BOLA 403 と区別）、計測0件の空表示、他ユーザー/不在デバイスの 404 によるユーザー列挙防止、HTMX(HX-Redirect)/非HTMX(303) の使い分け、全文言の日本語。
